...
class GaussianNB:
    ...
    def cal_stdev(self,X):
        return np.std(X,ddof=0)+self.epsilon  #非常重要！开始时未考虑导致预测准确率较低
    
    def Gaussian_prob(self,x,mean,stdev):
        return float(1/((np.sqrt(2*np.pi))*stdev)*np.exp(-0.5*pow(x-mean,2)/pow(stdev,2)))
    ...
    def predict(self,X):
        ...
        for x in X:
            ...
            for cls in self.classes:
                ...
                for i in range(X.shape[1]):
                    prob+=np.log(self.Gaussian_prob(x[i],self.mean[cls][i],self.stdev[cls][i])+self.epsilon)
                probs[cls]=prob
